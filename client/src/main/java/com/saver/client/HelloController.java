...
public class HelloController implements Initializable {
    ...
    public void download(ActionEvent actionEvent) throws IOException {
        network.getOs().writeUTF("#fileDOWNLOAD#");
        System.out.println("#fileDOWNLOAD# was written");
        String file = serverView.getSelectionModel().getSelectedItem();
        System.out.println("selected file: " + file);
        network.getOs().writeUTF(file);
        System.out.println(Path.of(homeDir).resolve(file));
        network.downloadFile(Path.of(homeDir).resolve(file).toString());
    }
